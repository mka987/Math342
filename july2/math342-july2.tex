...
\item Let $a_i$ be a nonzero residue which is not a root of $x^{h/p_i}-1$, and
  define $b_i=a_i^{h/p_i^{r_i}}$. Show that $\ord_p(b_i)=r_i$.
...
\item $\left( \frac{a}{p} \right) \equiv \pm1 \pmod{p}$; in particular, $\left(
    \frac{a}{p} \right) \equiv 1 \pmod{p}$ if and only if $a$ is a quadratic
  residue modulo $p$.
\end{enumerate*} \\
...
\item Show that $\left( \frac{2}{p} \right)(1+i)i^{(p-1)/2} \equiv
  ...
  \equiv (-1)^{(p\pm1)/2} \pmod{p}$ predicated upon whether $\frac{p-1}{2}$ is
  even or odd.
